...
class PaperFinder:
    ...
    def _handle_book(self, isbn: str, metadata: Dict, output_dir: Path, oa_callback=None, meta_callback=None) -> DownloadResult:
        ...
        # Browser callback for books (same as OA papers)
        def book_browser_callback(url: str):
            if oa_callback:
                oa_callback(isbn, url)
    ...
    def _register_dynamic_sources(self) -> None:
        ...
        for module_info in pkgutil.iter_modules(acquisition_pkg.__path__, acquisition_pkg.__name__ + "."):
            ...
            for _, obj in inspect.getmembers(module, inspect.isclass):
                ...
                def make_wrapper(source_cls):
                    def wrapper(doi, output_file, meta, _cls=source_cls, _session=self.session):
                        try:
    ...
    def acquire(self, doi_or_ref: str, output_dir: Optional[Path] = None, oa_callback=None, meta_callback=None) -> DownloadResult:
        """Alias for find() - for compatibility with test harness and other code."""
        return self.find(doi_or_ref, output_dir, oa_callback, meta_callback)

    def find(self, ref: str, output_dir: Optional[Path] = None, oa_callback=None, meta_callback=None) -> DownloadResult:
        ...
        if self.identity_resolver:
            ...
            if id_type == "isbn":
                # Book handling
                return self._handle_book(id_value, metadata, output_dir, oa_callback, meta_callback)
            elif id_type == "arxiv":
                # Fast-path arXiv
            ...
            if not doi and ref_str.replace('-', '').replace(' ', '').replace('X', '').isdigit():
                ...
                if len(potential_isbn) in [10, 13]:  # ISBN-10 or ISBN-13
                    ...
                    if book_metadata:
                        ...
                        return self._handle_book(book_metadata['isbn'], book_metadata, output_dir, oa_callback, meta_callback)
                    else:
