...
def download_pdf_with_fallbacks(doi: str, output_path: Path) -> bool:
    ...
    4. Publisher Direct - often fastest when available  
    5. PyPaperBot - multi-source aggregator
    6. Library Genesis (LibGen) - independent repository fallback
    7. arXiv - preprint server (specialized)
    8. Crossref - last-resort backup
    """
    session = _session_with_headers()
    ...
    # 2-4) Parallel fallback group: Open Access, Semantic Scholar, Publisher Direct
    # Try these in parallel with 20s timeout each; first success wins
    if doi:
        print("Trying legal sources in parallel (Unpaywall, Semantic Scholar, Publisher)...")
        ...
        def try_publisher():
            tmp = Path(tempfile.mkstemp(suffix=".pdf")[1])
            try:
                _try_fetch_publisher_direct(doi, tmp)
                return ("publisher", tmp)
            except Exception as e:
                try:
                    tmp.unlink()
                except Exception:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            futures = {
                ...
                executor.submit(try_publisher): "publisher",
            }
    ...
    # 6) Library Genesis (LibGen) - independent shadow library fallback
    if doi:
    ...
    # 7) arXiv - specialized for preprints
    if doi:
        try:
    ...
    # 8) Crossref Direct Links - last-resort backup
    if doi:
        try:
